...
public class Q11 {
    ...
    private static void quicksort(ListaDupla jogadores, CelulaDupla esq, CelulaDupla dir) {
        ...
        while (j.prox != i) {
            while ((i.elemento.getEstadoNascimento().compareTo(pivo.elemento.getEstadoNascimento()) < 0)
                    || (i.elemento.getEstadoNascimento().compareToIgnoreCase(pivo.elemento.getEstadoNascimento()) == 0
                            && i.elemento.getNome().compareToIgnoreCase(pivo.elemento.getNome()) < 0))
                i = i.prox;
            while ((j.elemento.getEstadoNascimento().compareTo(pivo.elemento.getEstadoNascimento()) > 0)
                    || (j.elemento.getEstadoNascimento().compareToIgnoreCase(pivo.elemento.getEstadoNascimento()) == 0
                            && j.elemento.getNome().compareToIgnoreCase(pivo.elemento.getNome()) > 0))
                j = j.ant;
            if (j.prox != i) {
                swap(i, j);
                if (i != j) {
