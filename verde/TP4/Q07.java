...
class Hash {
    ...
    public String pesquisar(Jogador elemento) {
        String resp = "NAO";
        int pos = h(elemento);
        if (tabela[pos].getNome().equals(elemento.getNome())) {
            resp = "SIM";
        } else if (tabela[pos] != null) {
            for (int i = 0; i < reserva; i++) {
                if (tabela[m1 + i] != null && tabela[m1 + i].getNome().equals(elemento.getNome())) {
                    resp = "SIM";
                    i = reserva;
                }
            }
    ...
    public void criarLog(String matricula, long tempoExecucao, int numComparacoes) {
        try {
            FileWriter fileWriter = new FileWriter(matricula + "800989_hashReserva.txt");
